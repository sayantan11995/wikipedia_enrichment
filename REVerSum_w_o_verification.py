...
text_file = open("results/REVerSum_w_o_verification.txt", "w")
...
for book, config in book_config.items():
    ...
    for ocr_type in ["tesseract"]:
        ...
        for section_name, section_content in tqdm(wikipedia_content.items()):
            ...
            messages = [
                {"from": "human", "value": relevance_prompt},
            ]
            ...
            relevant_documents_identified = "".join(generated_text)
            ...
            messages = [
                ...
                {"from": "human", "value": evidence_extraction_template},
            ]
            input_ids = tokenizer.apply_chat_template(
                messages,
                tokenize = True,
        ...
        with open(f"{output_json}/{book}/REVerSum_w_o_verification_llama3_instruct_{ocr_type}.json", "w") as content:
...
with open("results/REVerSum_w_o_verification_llama3_instruct.json", "w") as content:
    json.dump(verified_statements_summary_results, content)

# Initialize sums and counts
sums = {"informativeness": 0, "understandability": 0, "readability": 0, "quality": 0}
counts = {"informativeness": 0, "understandability": 0, "readability": 0, "quality": 0}

# Loop through data to calculate sums and counts
for source in verified_statements_summary_results.values():
    for metrics in source.values():
        if metrics:
            for key in sums:
                sums[key] += metrics.get(key, 0)
                counts[key] += 1 if key in metrics else 0
...
print("Averages:", formatted_averages)